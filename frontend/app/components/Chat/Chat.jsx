...
function Message({ role, children, avatarUrl, authorName }) {
  ...
  return (
    <div
      className={`message-row flex items-end gap-8 ${
...
function ChatMessages({ messages = [] }) {
  ...
  return (
    <div className="chat-messages overflow-hidden">
...
function ChatInput({ threadId, onMessageSent }) {
  ...
  return (
    <div className="chat-input-form-container flex justify-between items-center gap-8 w-full self-end">
      <Button
        type="icon"
        variant="glass"
        iconSize="md"
        icon={<Plus />}
        size="icon-md"
        onClick={() => alert("Feature: Add media")}
      />
      <form
        ref={formRef}
      ...
      <Button
        ...
        icon={<Mic />}
        size="icon-md"
        onClick={() => alert("Feature: Voice input")}
      />
    </div>
  );
}
