...
import { BookmarkIcon, ChartColumnBig, Circle, CircleQuestionMark, LogOut, Settings, Sparkles, Star, Users, X } from 'lucide-react';
...
export default function MenuOverlay() {
    ...
    return (
        <div className='fixed flex flex-col inset-0 bg-default z-50 p-12 gap-8' role="dialog" aria-modal="true" aria-labelledby="menu-overlay">
            <div className="flex items-center justify-between mb-12 w-full">
                <Button type="icon" variant="ghost" icon={<X />} />
                <h4 id="menu-overlay" className='text-h4 self-center'>Menu</h4>
                <span />
            </div>
            <div className='flex flex-col space-y-24 items-start w-full'>
                {MENU_ITEMS.map((item, idx) => {
                    ...
                    return (
                        ...
                        >
                            <h4 className={`text-h4 ${isDestructive ? 'color-destructive' : 'color-default'}`}>
                                {item.label}
                            </h4>
                        </Button>
                    );
