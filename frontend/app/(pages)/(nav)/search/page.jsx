...
const TABS = ["For you", "People", "Collaborations", "Services", "Tags"];
...
const errorMsg = (error) => <p className="text-base color-error">{error}</p>;

export default function SearchOverlay() {
  const router = useRouter();
  const handleClose = () => closeOverlay(router);
  const [activeTab, setActiveTab] = useState(null);
  const { query, setQuery, results, isLoading, error, recentSearches } =
    useSearch({
      minLength: 1,
      debounceMs: 300,
      mapResponse: (data) => ({
    ...
    return (
      <div className="flex flex-col gap-4">
        <section className="flex flex-col gap-8">
          ...
          <div className="flex flex-col gap-4">{items.map(renderUserItem)}</div>
        </section>
      </div>
    );
  };
  ...
  const forYouData = useMemo(
    () => results?.forYou || EMPTY_RESULTS.forYou,
    [results]
  );
  ...
  const renderForYou = () => {
    ...
    const hasAny =
      people.length || collaborations.length || services.length || tags.length;
    const trimmedQuery = query.trim();
